...
class JSONWebSignatureSerializer(Serializer):
    ...
    def load_payload(self, payload, serializer=None, return_header=False):
        ...
        try:
            header = Serializer.load_payload(
                self, json_header, serializer=json
            )
        except BadData as e:
            raise BadHeader(
        ...
        if not isinstance(header, dict):
            raise BadHeader(
                "Header payload is not a JSON object", header=header
            )
        payload = Serializer.load_payload(
            self, json_payload, serializer=serializer
        )
        if return_header:
    ...
    def loads(self, s, salt=None, return_header=False):
        ...
        if header.get("alg") != self.algorithm_name:
            raise BadHeader(
                "Algorithm mismatch", header=header, payload=payload
            )
        if return_header:
            return payload, header
        return payload
...
class TimedJSONWebSignatureSerializer(JSONWebSignatureSerializer):
    ...
    def loads(self, s, salt=None, return_header=False):
        ...
        int_date_error = BadHeader(
            "Expiry date is not an IntDate", payload=payload
        )
        try:
