            ...
            return sum(
                [
                    ...
                    * difflib.SequenceMatcher(
                    ...
                    0.01
                    * bool(set(self.values or ()).issubset(rule.arguments)),
...
class Rule(RuleFactory):
    ...
    def __init__(
        ...
        self._trace = (
            self._converters
        ) = self._regex = self._argument_weights = None
    ...
    def bind(self, map, rebind=False):
        ...
        if self.map is not None and not rebind:
            raise RuntimeError(
                "url rule %r already bound to map %r" % (self, self.map)
            )
        self.map = map
            ...
            if is_dynamic and data in defaults:
                ...
                opl.append(
                    (
                        False,
                        url_quote(
                            to_bytes(data, self.map.charset), safe="/:|+"
                        ),
                    )
                )
            else:
    ...
    def __eq__(self, other):
        return (
            self.__class__ is other.__class__ and self._trace == other._trace
        )
...
class MapAdapter(object):
    ...
    def get_default_redirect(self, rule, method, values, query_args):
        ...
        for r in self.map._rules_by_endpoint[rule.endpoint]:
            ...
            if r is rule:
                break
            if r.provides_defaults_for(rule) and r.suitable_for(
                values, method
            ):
