...
class ViewItems(object):
    ...
    def __get_items(self):
        return getattr(self.__multi_dict, self.__method)(
            *self.__a, **self.__kw
        )
...
class CombinedMultiDict(ImmutableMultiDictMixin, MultiDict):
    ...
    def fromkeys(cls):
        raise TypeError(
            "cannot create %r instances by fromkeys" % cls.__name__
        )
...
class Range(object):
    ...
    def __init__(self, units, ranges):
        ...
        for start, end in ranges:
            if start is None or (
                end is not None and (start < 0 or start >= end)
            ):
                raise ValueError(
                    "{} is not a valid range.".format((start, end))
                )
