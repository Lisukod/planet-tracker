    ...
    def encode_netloc(self):
        ...
        auth = ":".join(
            filter(
                ...
                [
                    url_quote(
                        self.raw_username or "", "utf-8", "strict", "/:%"
                    ),
                    url_quote(
                        self.raw_password or "", "utf-8", "strict", "/:%"
                    ),
                ],
            )
        )
...
def _url_unquote_legacy(value, unsafe=""):
    try:
        return url_unquote(
            value, charset="utf-8", errors="strict", unsafe=unsafe
        )
    except UnicodeError:
...
def url_decode_stream(
    ...
    decoder = _url_decode_impl(
        pair_iter, charset, decode_keys, include_empty, errors
    )
