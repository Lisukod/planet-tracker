...
"""
    ...
    if len(gui) > 0:
        generated.extend(
            maker.make_multiple(
                ["%s = %s" % kv for kv in gui.items()], {"gui": True}
            )
        )
...
class WheelBuilder(object):
    ...
    def _build_one(self, req, output_dir, python_tag=None):
        ...
        with req.build_env:
            return self._build_one_inside_env(
                req, output_dir, python_tag=python_tag
            )
    ...
    def _base_setup_args(self, req):
        ...
        base_cmd = make_setuptools_shim_args(
            req.setup_py_path, unbuffered_output=True
        )
    ...
    def _build_one_legacy(self, req, tempd, python_tag=None):
        ...
        with open_spinner(spin_message) as spinner:
            logger.debug("Destination directory: %s", tempd)
            wheel_args = (
                base_args + ["bdist_wheel", "-d", tempd] + self.build_options
            )
