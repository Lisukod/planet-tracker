...
PATH_CHARS = (
    abnf_regexp.UNRESERVED_CHARS_SET
    | abnf_regexp.SUB_DELIMITERS_SET
    | {":", "@", "/"}
)
...
def parse_url(url):
    ...
    if (
        uri_ref.scheme is None
        or uri_ref.scheme.lower() in NORMALIZABLE_SCHEMES
    ):
    ...
    validator = Validator()
    try:
        validator.check_validity_of(*validator.COMPONENT_NAMES).validate(
            uri_ref
        )
